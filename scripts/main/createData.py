...
def create_anno(canvas, members_map, odir, index, info):
  ...
  for i in range(len(members)):
    ...
    if "新編日本古典文学全集" in member_label:
      ...
      chars = "新編日本古典文学全集 p."+str(page)+" 開始位置<p><a href=\"https://gateway2.itc.u-tokyo.ac.jp:11039/lib/display/?lid="+str(sagaId)+"\" target=\"_blank\" rel=\"noopener noreferrer\">ジャパンナレッジ</a>でみる</p>"
      fill = "#2E89D9"
      stroke = "#2E89D9"
    else:
      ndlId = info["ndl"].split("/")[-2]
      ndlPage = info["ndl_front"] + round((page - info["koui_front"]) / 2)
      chars = "源氏物語大成 p."+str(page)+" 開始位置<p><a href=\"http://dl.ndl.go.jp/info:ndljp/pid/"+ndlId+"/"+str(ndlPage)+"\" target=\"_blank\" rel=\"noopener noreferrer\">国立国会図書館デジタルコレクション</a>で校異源氏物語をみる</p>"
      fill = "#F3AA00"
      stroke = "#A52A2A"
    
    svg = "<svg xmlns='http://www.w3.org/2000/svg'><path xmlns=\"http://www.w3.org/2000/svg\" d=\"M" + str(x) + "," + str(y) + "c0,-" + str(d2 * 2) + " " + str(d2) + ",-" +str(d2 * 4) + " " +str(d2 * 3) + ",-" + str(d2 * 6) + "c0,-" + str(d2 * 2) + " -" + str(d2) + ",-" + str(d2 * 3) + " -" + str(d2 * 3) + ",-" + str(d2 * 3) + "c-" + str(d2 * 2) + ",0 -" + str(d2 * 3) + "," + str(d2) + " -" + str(d2 * 3) + "," + str(d2 * 3) + "c" + str(d2 * 2) + "," + str(d2 * 2) + " " + str(d2 * 3) + "," + str(d2 * 4) + " " + str(d2 * 3) + "," + str(d2 * 6) + "z\" id=\"pin_" + "abc" + "\" fill=\""+fill+"\" stroke=\""+stroke+"\"/></svg>"
...
def create_manifest(selection, info):
  ...
  if "東大本" in label:
    curation_uri = "https://raw.githubusercontent.com/nakamura196/genji_curation/master/docs/iiif/saga/"+str(vol).zfill(2)+".json"
    try:
  ...
  if response.status_code < 400:
    ...
    for surface in surfaces:
      ...
      for zone in zones:
        ...
        line_id = "https://kouigenjimonogatari.github.io/data/"+zone_id.split("_")[1]+"-01.json"
