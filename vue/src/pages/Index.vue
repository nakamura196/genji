<template>
    <div style="width : 100%; height : 800px; background-color: LightGray;">
        ...
        <VueDragResize :isActive="true" :x="0" :y="0" :w="600" :h="750" v-on:resizing="resize" v-on:dragging="resize" class="p-2" :sticks="['ml', 'mr']">
            <div class="card">
                <div class="card-body">
                    <iframe :src="mirador_path" seamless width="100%" height="700px" style="border: none;"></iframe>
                </div>
            </div>
        </VueDragResize>
    
        <VueDragResize :isActive="true" :x="600" :y="0" :w="600" :h="400" v-on:resizing="resize" v-on:dragging="resize" class="p-2">
            <div class="card scroll" v-bind:class='{vertical: direction == "vertical"}' id="org">
                <div class="card-body">
                    <h5><b>翻刻文</b></h5>
                    <div :style="line.selected ? 'background-color : yellow;' : null" v-for="(line, line_id) of lines_org" v-bind:key="line_id" :id="line_id" v-on:mouseover="mouseover" @click="clickSmoothScroll(line_id)">
                        <img v-if="line.page" @click="mirador_path = line.page" src="https://iiif.dl.itc.u-tokyo.ac.jp/images/iiif.png" class="m-2" />
    
                        <template v-for="node of line.array">{{ node.text }}
</template>
            </div>
        </div>
      </div>
    ...
    <VueDragResize
      ...
      class="p-2"
    >
      <div class="card scroll" v-bind:class='{vertical: direction == "vertical"}' id="modern">
        <div class="card-body">
          <h5><b>現代語訳</b></h5>
          <div
            ...
            if (modern_ids != null) {
                if (this.direction == "vertical") {
                    ...
                    this.$SmoothScroll(
                        ...
                        .left +
                        document.querySelector("#modern").scrollLeft -
                        document.querySelector("#modern").getBoundingClientRect().left - document.querySelector("#modern").getBoundingClientRect().width / 2 + document.querySelector("#" + modern_ids[0]).getBoundingClientRect().width,
                        400,
                        null,
            ...
            if (modern_ids != null) {
                if (this.direction == "vertical") {
                    this.$SmoothScroll(
                        document.querySelector("#" + modern_ids[0]).getBoundingClientRect()
                        .left +
                        document.querySelector("#org").scrollLeft -
                        document.querySelector("#org").getBoundingClientRect().left - document.querySelector("#org").getBoundingClientRect().width / 2 + document.querySelector("#" + modern_ids[0]).getBoundingClientRect().width,
                        400,
                        null,
